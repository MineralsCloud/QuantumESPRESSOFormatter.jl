...
!!! info
    The style guide is always a work in progress, and not all QuantumESPRESSOFormatter code
    follows the rules. When modifying QuantumESPRESSOFormatter, please fix the style violations
...
QuantumESPRESSOFormatter uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/QuantumESPRESSOFormatter.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the QuantumESPRESSOFormatter directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to QuantumESPRESSOFormatter have
